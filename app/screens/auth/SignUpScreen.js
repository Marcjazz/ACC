...
import { registerUser } from '../../api-backend/auth.service'
...
const Form = () => {
  ...
  const register = () => {
    registerUser({ email, contact, password })
      .then(() => {
        //TODO enregistrer avec success
      })
      .catch((error) => {
        Alert.alert(error?.messsage || `Une erreur s'est produite`)
      })
  }
